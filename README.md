...
Яндекс может в любой момент поменять дизайн своих карт или той части что отвечает за Транспорт. Поскольку данный скрипт это парсер, скорее всего в таком случае работать он перестанет. Один раз это уже произошло, в процессе разработки (отсюда 1.1.Х в версии, а не 1.0.Х).
Новая версия проекта, Yandex Transport Proxy работает по другому принципу, и более защищена от таких событий.

На данный момент поддержка данного "чудовища" прекращена, мониторинг просто оставлен для экспермиента.
